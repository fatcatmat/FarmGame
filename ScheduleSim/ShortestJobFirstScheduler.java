public class ShortestJobFirstScheduler extends Scheduler
{
    ...
    public void tick() {
        ...
        for(job = cpu.readyQueue.first(); job != null; job = job.next()){ //traverses through a
            if(job.estimatedLength() < shortestTime){
                shortestJob = job;
                shortestTime = shortestJob.estimatedLength();
            }
        }

        //this is kept because this condition we are waiting for the jobs to be completed, where in the time remaining we are anticipating several switches as it slowly finishes steps
